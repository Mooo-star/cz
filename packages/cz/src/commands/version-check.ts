...
export const checkVersion = async (): Promise<void> => {
  ...
  try {
    ...
    if (latestVersion && compareVersions(version, latestVersion)) {
      ...
      console.log(chalk.bold(chalk.yellow('│                                                       │')));
      console.log(chalk.bold(chalk.yellow('│  ✨ 更新提示                                          │')));
      console.log(chalk.bold(chalk.yellow(`│  发现新版本 ${chalk.green(latestVersion)} (当前版本 ${chalk.red(version)})${' '.repeat(20)}│`)));
      console.log(chalk.bold(chalk.yellow(`│  运行 ${chalk.cyan(updateCommand)} 更新到最新版本  │`)));
      console.log(chalk.bold(chalk.yellow('│                                                       │')));
      console.log(chalk.bold(chalk.yellow('╰───────────────────────────────────────────────────────╯')));
      console.log();
    }
